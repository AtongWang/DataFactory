...
from backend.models.knowledge_graph_models import KnowledgeGraph, NodeType, RelationshipType, Base
...
def safe_create_completion_for_kg(client, completion_params, model_name="unknown"):
    ...
    try:
        ...
        if 'provider' in error_msg and ('unexpected' in error_msg or 'invalid' in error_msg):
            logger.warning(f"知识图谱管理器: 模型 {model_name} 不支持provider参数，尝试移除provider参数后重试")
            ...
            try:
                ...
                logger.info(f"知识图谱管理器: 模型 {model_name} 成功使用fallback请求（无provider参数）")
                return response
            except Exception as fallback_e:
                logger.error(f"知识图谱管理器: 模型 {model_name} fallback请求也失败: {str(fallback_e)}")
                raise fallback_e
        else:
            # 其他类型的错误，直接抛出
            raise e
...
class KnowledgeGraphManager:
        ...
        try:
            ...
            if database_config:
                ...
                if db_type == 'mysql':
                    ...
                    """
                    cursor.execute(query, (table_name, database_config.get('database', '')))
    ...
    def get_knowledge_graph(self, graph_id):
        ...
        session = self.Session()
        try:
            graph = session.query(KnowledgeGraph).filter(KnowledgeGraph.id == graph_id).first()
        ...
        try:
            ...
            for node_type_data in graph_data.get('node_types', []):
                ...
                if not isinstance(identifier_cols, list):
                     identifier_cols = [identifier_cols] if identifier_cols else [] # Ensure it's a list

                # Prepare split_config
                split_config = node_type_data.get('split_config', {'enabled': False, 'delimiter': None})
                split_config_json = json.dumps(split_config) if split_config and split_config.get('enabled') else None

                node_type = NodeType(
                    knowledge_graph_id=graph_id,
                    name=node_type_data['name'],
                    identifier_columns=json.dumps(identifier_cols), # Save as list
                    attribute_columns=json.dumps(node_type_data.get('attribute_columns', [])),
            ...
            for rel_data in graph_data.get('relationships', []):
                ...
                if not source_node or not target_node:
                    logger.warning(f"跳过关系 {rel_data['type']}: 源节点或目标节点不存在")
                    continue
                
                # 存储跨行匹配配置
                inter_row_config = None
                if rel_data.get('matching_mode') == 'inter-row' and rel_data.get('inter_row_options'):
            ...
            # 验证数据完整性
            logger.info(f"图谱 {graph_id} 创建了 {len(node_types_created)} 个节点类型，{len(relationships_created)} 个关系类型")
        ...
        try:
            with self.neo4j_driver.session() as neo4j_session:
                ...
                while True:
                    ...
                    logger.info(f"从 Neo4j 中删除了 {deleted_count} 个 graph_id={graph_id} 的节点和它们的关系")
                    if deleted_count == 0:
                        break
    ...
    def start_graph_building(self, graph_id):
        ...
        try:
            ...
            if graph.status in ('building', 'completed'):
                ...
                return {'status': 'skipped', 'message': f'图谱状态为 {graph.status}，跳过构建'}
    ...
    def _build_knowledge_graph(self, graph_id):
        ...
        try:
            ...
            if graph_id in self.building_tasks and self.building_tasks[graph_id].get('cancelled'):
            ...
            node_types_list = session.query(NodeType).filter(NodeType.knowledge_graph_id == graph_id).all()
            relationship_types = session.query(RelationshipType).filter(RelationshipType.knowledge_graph_id == graph_id).all()
            ...
            for node_type in node_types_list:
                ...
                attr_columns = json.loads(node_type.attribute_columns) if node_type.attribute_columns else []
                # Load split_config
                split_config = json.loads(node_type.split_config) if node_type.split_config else None
                
                # 创建该类型的实体, 传递 split_config 和 graph_id
                entities = self._create_entities(table_data, entity_type, id_columns, attr_columns, split_config, graph_id)
            ...
            for rel_type in relationship_types:
                ...
                if not source_node_type or not target_node_type:
                    logger.warning(f"跳过关系 {rel_type.type}: 找不到源节点类型ID {rel_type.source_node_type_id} 或目标节点类型ID {rel_type.target_node_type_id} 对应的对象")
                    continue
                ...
                if rel_type.matching_mode == 'intra-row':
                    ...
                    new_relationships = self._create_intra_row_relationships(
                        table_data, source_entities, target_entities, 
                        source_node_type, target_node_type, rel_type
                    )
                else:
                    # 跨行匹配
                    ...
                    try:
                        ...
                        if not inter_row_config:
                            logger.warning(f"关系 {rel_type.id} 跨行匹配配置为空或无效，跳过")
                            continue
                            
                        new_relationships = self._create_inter_row_relationships(
                            table_data, source_entities, target_entities,
                            source_node_type, target_node_type, rel_type, # Pass NodeType objects
                            inter_row_config
                        )
                    except Exception as e:
                        logger.error(f"处理关系 {rel_type.type} 跨行匹配配置时出错: {str(e)}")
                        continue
            ...
            success = self._export_graph_data_to_neo4j(graph_id, entity_store, relationships_data)

            if success:
                 graph.status = 'completed'
                 logger.info(f"图谱 {graph_id} 构建并保存到 Neo4j 完成: {node_count} 个节点, {relationship_count} 个关系")
                 ...
                 try:
                     ...
                     if self.neo4j_driver:
                         with self.neo4j_driver.session(database="neo4j") as final_session:
                             # 强制schema刷新
                     ...
                     logger.warning(f"图谱 {graph_id} 最终schema刷新失败，但不影响图谱构建: {final_refresh_error}")
            else:
                 graph.status = 'failed'
    ...
    def _export_graph_data_to_neo4j(self, graph_id, entity_store, relationships):
        ...
        try:
            with self.neo4j_driver.session(database="neo4j") as neo4j_session: # Specify database if needed
                
                # 0. 先删除该 graph_id 的旧数据 - 更彻底的清理
                logger.info(f"开始清理 Neo4j 中 graph_id={graph_id} 的旧数据...")
                self._delete_graph_data_from_neo4j_thorough(graph_id, neo4j_session) # 使用更彻底的清理方法
                ...
                for entity_type in all_entity_types:
                     ...
                     try:
                         neo4j_session.run(f"CREATE CONSTRAINT IF NOT EXISTS FOR (n:{escaped_label}) REQUIRE n.unique_id IS UNIQUE")
                         ...
                         logger.warning(f"创建 Neo4j 约束时出错 ({escaped_label}): {constraint_e}. 导入仍将继续。")
                ...
                for entity_type, entities in entity_store.items():
                     for entity_id, entity_data in entities.items():
                          # 检查是否有重复的unique_id
                          unique_id = entity_data['id'] # Use the 'EntityType:Identifier' string
                          ...
                          properties['name'] = unique_id.split(':', 1)[-1] # Extract name after type:
                          # Convert non-primitive types like lists/dicts if necessary, though attributes seem basic
                          for k, v in properties.items():
                              if isinstance(v, (list, dict)):
                                  properties[k] = json.dumps(v) # Store complex types as JSON strings
                ...
                logger.info(f"数据去重完成，原始实体数: {sum(len(entities) for entities in entity_store.values())}，去重后: {len(entities_flat)}")
                    ...
                    if apoc_available:
                        ...
                        try:
                            ...
                            neo4j_session.run("""
                                ...
                                RETURN count(node)
                            """, batch=batch, graph_id=graph_id)
                            logger.info(f"写入了 {len(batch)} 个节点到 Neo4j (APOC，批次 {i // node_creation_batch_size + 1})")
                            continue  # 成功则继续下一批次
                        except Exception as apoc_e:
                            logger.error(f"APOC 方法失败，切换到备用方案: {apoc_e}")
                            apoc_available = False  # 标记APOC不可用，后续批次直接使用备用方案
                    
                    # 备用方案：不使用APOC，按标签分组创建
                    logger.info(f"使用备用方案 (非APOC) 处理批次 {i // node_creation_batch_size + 1}")
                    try:
                        # 按标签分组
                        nodes_by_label = {}
                        for node in batch:
                            label = node['labels'][0] if node['labels'] else 'DefaultLabel'
                        ...
                        for label, properties_list in nodes_by_label.items():
                            ...
                            neo4j_session.run(f"""
                                ...
                                SET n = props
                            """, properties_list=properties_list, graph_id=graph_id)
                            logger.info(f"备用方案写入了 {len(properties_list)} 个 {label} 节点")
                        ...
                        for j, node in enumerate(batch):
                            try:
                                ...
                                logger.error(f"单个节点写入失败 {node['properties']['unique_id']}: {single_e}")
                                # 继续处理其他节点
                ...
                for rel in relationships:
                    ...
                    if not isinstance(properties, dict): properties = {} # Ensure it's a dict
                    properties['graph_id'] = graph_id 
                    ...
                    rels_to_create.append({
                        ...
                        'type': rel['type'].replace('-', '_').replace(' ', '_'), # Sanitize relationship type for Neo4j
                        'properties': properties
                    })

                logger.info(f"关系去重完成，原始关系数: {len(relationships)}，去重后: {len(rels_to_create)}")
                     ...
                     if apoc_available:
                         ...
                         try:
                             ...
                             neo4j_session.run(f"""
                                 ...
                                 RETURN count(rel)
                             """, batch=batch, graph_id=graph_id)
                             logger.info(f"写入了 {len(batch)} 个关系到 Neo4j (APOC，批次 {i // rel_creation_batch_size + 1})")
                             continue  # 成功则继续下一批次
                         except Exception as apoc_e:
                             logger.error(f"关系 APOC 方法失败，切换到备用方案: {apoc_e}")
                             apoc_available = False  # 标记APOC不可用
                     
                     # 备用方案：不使用APOC，直接创建关系
                     logger.info(f"使用备用方案 (非APOC) 处理关系批次 {i // rel_creation_batch_size + 1}")
                     try:
                         # 按关系类型分组以提高效率
                         rels_by_type = {}
                         for rel in batch:
                         ...
                         for rel_type, rels in rels_by_type.items():
                             ...
                             """, rels=rels, graph_id=graph_id)
                             logger.info(f"备用方案写入了 {len(rels)} 个 {rel_type} 关系")
                         ...
                         for rel in batch:
                             try:
                                 ...
                                 logger.error(f"单个关系创建失败 {rel['source_id']}->{rel['target_id']}: {single_rel_e}")
                                 # 继续处理其他关系
                ...
                try:
                    ...
                    logger.warning(f"刷新Neo4j schema缓存失败，但不影响数据写入: {refresh_error}")
    ...
    def _delete_graph_data_from_neo4j_thorough(self, graph_id, neo4j_session=None):
        ...
        try:
            ...
            if remaining_count > 0:
                logger.warning(f"仍有 {remaining_count} 个节点未被删除，尝试强制清理...")
                # 尝试强制删除
                neo4j_session.run("""
                    MATCH (n {graph_id: $graph_id})
                    DETACH DELETE n
            ...
            logger.info(f"彻底清理完成，共删除 {deleted_total} 个 graph_id={graph_id} 的节点")
        except Exception as e:
    ...
    def _fetch_table_data(self, database_name, table_name):
        ...
        try:
            ...
            if (database_name and 
                ('/' in database_name or '\\' in database_name or 
                 database_name.endswith('.sqlite') or database_name.endswith('.db'))):
                # 这是一个SQLite文件路径
                database_config = {
                    'type': 'sqlite',
                ...
                database_config = {
                    ...
                    'database': database_name or self.vanna_manager.config.database.database_name
                }
    ...
    def _create_entities(self, table_data, entity_type, id_columns, attr_columns, split_config=None, graph_id=None):
        """
        创建实体并返回实体字典
        ...
        if split_enabled and not delimiter:
             logger.warning(f"实体类型 '{entity_type}' 启用了拆分但未提供分隔符，将禁用拆分。")
             split_enabled = False
        if split_enabled and not split_column:
             logger.warning(f"实体类型 '{entity_type}' 启用了拆分但未提供有效的标识符列，将禁用拆分。")
             split_enabled = False

        logger.info(f"开始为实体类型 '{entity_type}' 创建实体，分拆模式: {split_enabled}")
        ...
        for index, row in table_data.iterrows():
            ...
            if split_enabled:
                ...
                if raw_value and isinstance(raw_value, str):
                    ...
                    if delimiter in raw_value:
                        split_values = [val.strip() for val in raw_value.split(delimiter) if val.strip()]
                    else:
                ...
                elif raw_value: # Handle non-string but non-empty values (e.g., numbers)
                ...
                id_values = [str(row[col]) for col in id_columns if col in row and pd.notna(row[col])]
                if id_values: # Only proceed if we have some identifier values
                     ...
                     identifier = ':'.join(id_values) if len(id_columns) > 1 else id_values[0]
            ...
            for identifier_value in entity_identifiers:
                 # 过滤空白或无效的标识符
                 if not identifier_value or str(identifier_value).strip() == '' or str(identifier_value) == 'nan':
                     empty_identifier_count += 1
                     continue
                 ...
                 if entity_id in entities:
                     ...
                     if index not in entities[entity_id]['row_indices']:
                         ...
                         for k, v in attributes.items():
                             if k not in entities[entity_id]['attributes'] or pd.isna(entities[entity_id]['attributes'][k]):
                                 entities[entity_id]['attributes'][k] = v
                     else:
                         logger.debug(f"实体 {entity_id} 在同一行 {index} 重复创建，跳过")
                 else:
                     # Create new entity
                     entities[entity_id] = {
                         'id': entity_id,
        ...
        if duplicate_count > len(entities) * 2:
            logger.warning(f"实体类型 '{entity_type}' 的重复率异常高 ({duplicate_count}/{len(entities)})，请检查标识符列配置")
        
        return entities
    
    def _create_intra_row_relationships(self, table_data, source_entities, target_entities, 
                                   source_node_type, target_node_type, rel_type):
        """创建同行匹配的关系"""
        relationships = []
                    ...
                    for target_id in target_by_row[idx]:
                        ...
                        if rel_key not in relationship_set:
                            ...
                            if rel_type.direction == 'bi':
                                reverse_rel_key = f"{target_id}|{source_id}|{rel_type.type}"
    ...
    def _create_inter_row_relationships(self, table_data, source_entities, target_entities,
                                       source_node_type, target_node_type, rel_type, 
                                       inter_row_config):
        """创建跨行匹配的关系"""
        relationships = []
        ...
        for group_key, row_indices in groups.items():
            ...
            rule_relationships = self._apply_rule_conditions(
                ...
                rel_type, inter_row_config.get('rules', []), source_node_type, target_node_type,
    ...
    def _apply_rule_conditions(self, table_data, row_indices, source_entities, target_entities,
                              rel_type, rules, source_node_type, target_node_type, 
                              embedding_cache): # 添加 embedding_cache 参数
        ...
        for source_data in source_entities:
            for target_data in target_entities:
                ...
                match_result = self._evaluate_rule_items(rules, source_row, target_row, embedding_cache, source_row_idx, target_row_idx)

                # 如果满足条件，为 complex object 内的每个 string ID 对创建关系
                if match_result['matched']:
                    for s_entity_id_str in source_data['entities']: # Iterate string IDs
                        for t_entity_id_str in target_data['entities']: # Iterate string IDs
                            # 添加自环检查 (修正语法 - 使用括号)
                            if (source_node_type and target_node_type and
                                source_node_type.name == target_node_type.name and
                                s_entity_id_str == t_entity_id_str):
                                continue
                            ...
                            if match_result['semantic_info']:
                                ...
                                if len(match_result['semantic_info']) == 1:
                                    ...
                                    properties['similarity_score'] = semantic['similarity_score']
                                    properties['semantic_columns'] = ','.join(semantic['columns'])
                                    properties['similarity_threshold'] = semantic['threshold']
                                else:
                                    # 多个语义条件，取最高相似度
                                    max_similarity = max(s['similarity_score'] for s in match_result['semantic_info'])
                                    all_columns = []
                                    all_thresholds = []
                                    ...
                                    properties['semantic_columns'] = ','.join(set(all_columns))  # 去重
                                    properties['similarity_threshold'] = max(all_thresholds)
                                    properties['semantic_count'] = len(match_result['semantic_info'])
                                
                                relationship['properties'] = properties
                                
                                # 记录语义匹配日志
                                logger.info(f"创建语义匹配关系: {s_entity_id_str} -> {t_entity_id_str}, 相似度: {properties['similarity_score']}")
                            
                            rel_key = f"{s_entity_id_str}|{t_entity_id_str}|{rel_type.type}"
                            ...
                            if rel_type.direction == 'bi':
                                ...
                                # 如果有语义匹配信息，也添加到反向关系中
                                if match_result['semantic_info']:
                                    reverse_relationship['properties'] = relationship.get('properties', {}).copy()
                                
                                reverse_rel_key = f"{t_entity_id_str}|{s_entity_id_str}|{rel_type.type}"
                                if reverse_rel_key not in relationship_set: # 检查反向关系是否已存在
                                    relationships.append(reverse_relationship)
                                    relationship_set.add(reverse_rel_key)

        return relationships
    
    def _evaluate_rule_items(self, rule_items, source_row, target_row, embedding_cache, source_row_idx, target_row_idx): # 添加 cache 和 idx 参数
        ...
        if first_item['type'] == 'rule':
            ...
            semantic_result = self._evaluate_single_semantic_condition(first_item, source_row, target_row, embedding_cache, source_row_idx, target_row_idx)
            ...
            result = self._evaluate_single_rule(first_item, source_row, target_row)
             ...
             group_result = self._evaluate_rule_items(first_item['items'], source_row, target_row, embedding_cache, source_row_idx, target_row_idx)
        ...
        for i in range(1, len(rule_items)):
            item = rule_items[i]
            logic_op = item.get('logic_operator', 'AND') # 获取与前一项的连接符，默认为AND
            ...
            if item['type'] == 'rule':
                current_result = self._evaluate_single_rule(item, source_row, target_row)
            elif item['type'] == 'semantic': # 处理语义条件
                semantic_result = self._evaluate_single_semantic_condition(item, source_row, target_row, embedding_cache, source_row_idx, target_row_idx)
                if isinstance(semantic_result, dict):
                ...
                group_result = self._evaluate_rule_items(item['items'], source_row, target_row, embedding_cache, source_row_idx, target_row_idx)
    ...
    def _evaluate_single_semantic_condition(self, condition, source_row, target_row, embedding_cache, source_row_idx, target_row_idx):
        """评估单个语义条件
        ...
        if source_row_idx in embedding_cache and col_combo_key in embedding_cache[source_row_idx]:
             ...
             if source_text:
                 ...
                 if embeddings_list and len(embeddings_list) > 0:
                      ...
                      # 存入缓存
                      if source_row_idx not in embedding_cache: embedding_cache[source_row_idx] = {}
                      embedding_cache[source_row_idx][col_combo_key] = source_embedding
                 else: logger.warning(f"无法计算源行 {source_row_idx} 的嵌入 ({columns})")
             else: logger.warning(f"无法为源行 {source_row_idx} 生成文本 ({columns})")

        # 获取或计算目标嵌入
        target_embedding = None
        if target_row_idx in embedding_cache and col_combo_key in embedding_cache[target_row_idx]:
             ...
             if target_text:
                 ...
                 if embeddings_list and len(embeddings_list) > 0:
                      ...
                      # 存入缓存
                      if target_row_idx not in embedding_cache: embedding_cache[target_row_idx] = {}
                      embedding_cache[target_row_idx][col_combo_key] = target_embedding
                 else: logger.warning(f"无法计算目标行 {target_row_idx} 的嵌入 ({columns})")
             else: logger.warning(f"无法为目标行 {target_row_idx} 生成文本 ({columns})")
    ...
    def _evaluate_single_rule(self, rule, source_row, target_row):
        ...
        if rule.get('type') == 'inter_entity_compare':
            ...
            # 获取两个实体的列值
            source_value = source_row[source_column] if source_column in source_row else None
            target_value = target_row[target_column] if target_column in target_row else None
            
            # 使用实体间比较方法
            return self._check_inter_entity_condition(source_value, target_value, operator)
    ...
    def _check_rule_condition(self, source_value, target_value, operator, compare_value):
        """检查规则条件是否满足
        ...
        try:
            ...
            logger.warning(f"规则条件比较错误: {str(e)}，值: {value}，比较值: {compare_value}，操作符: {operator}")
            return False
    ...
    def _check_inter_entity_condition(self, source_value, target_value, operator):
        ...
        try:
            ...
            if operator == '==':
                ...
                return self._compare_values_numeric(source_value, target_value, operator)
            elif operator == 'in':
            ...
            logger.warning(f"实体间比较条件比较错误: {str(e)}，源值: {source_value}，目标值: {target_value}，操作符: {operator}")
            return False
    ...
    def _compare_values_numeric(self, source_value, target_value, operator):
        ...
        try:
            ...
            try:
                ...
                logger.info(f"回退到字符串比较: '{source_str}' {operator} '{target_str}'")
            ...
            # 4. 最后的回退：无法比较
            logger.warning(f"无法比较值: {source_value} ({type(source_value)}) {operator} {target_value} ({type(target_value)})")
            return False
    ...
    def _get_database_connection(self, database_config):
        ...
        try:
            ...
            if db_type == 'mysql':
                ...
                if '/' in database_name or '\\' in database_name or database_name.endswith('.sqlite') or database_name.endswith('.db'):
    ...
    def calculate_graph_metrics(self, graph_id):
        ...
        try:
            with self.neo4j_driver.session(database="neo4j") as session:
                ...
                node_count_res = session.run("MATCH (n {graph_id: $graph_id}) RETURN count(n) as count", graph_id=graph_id).single()
                metrics['node_count'] = node_count_res['count'] if node_count_res else 0

                rel_count_res = session.run("MATCH (n {graph_id: $graph_id})-[r]->(m {graph_id: $graph_id}) RETURN count(r) as count", graph_id=graph_id).single()
                metrics['relationship_count'] = rel_count_res['count'] if rel_count_res else 0
                ...
                node_type_dist_res = session.run("""
                    ...
                    ORDER BY count DESC
                """, graph_id=graph_id)
                metrics['node_type_distribution'] = {record['type']: record['count'] for record in node_type_dist_res}
                ...
                rel_type_dist_res = session.run("""
                    ...
                    ORDER BY count DESC
                """, graph_id=graph_id)
                metrics['relationship_type_distribution'] = {record['type']: record['count'] for record in rel_type_dist_res}
                ...
                if metrics['node_count'] > 0:
                    ...
                    metrics['average_degree'] = round(avg_degree_res['avg_degree'], 2) if avg_degree_res and avg_degree_res['avg_degree'] is not None else 0.0
                    ...
                    degree_dist_res = session.run("""
                        ...
                        ORDER BY degree ASC
                    """, graph_id=graph_id)
                    metrics['degree_distribution'] = {record['degree']: record['node_count'] for record in degree_dist_res}

                    # 图密度 (有向图近似)
                    if metrics['node_count'] > 1:
                        max_edges = metrics['node_count'] * (metrics['node_count'] - 1)
                        metrics['graph_density'] = round(metrics['relationship_count'] / max_edges, 5) if max_edges > 0 else 0
                    else:
                         metrics['graph_density'] = 0.0 # Density is 0 for single node or no nodes
                else:
                ...
                """, graph_id=graph_id).single()
                metrics['orphan_node_count'] = orphan_nodes_res['count'] if orphan_nodes_res else 0
                logger.info("数据质量指标计算完成。")

                # --- 初始化 GDS 相关指标 ---
                metrics['community_detection'] = {'available': gds_available, 'calculated': False, 'error': None}
                metrics['degree_centrality'] = {'available': gds_available, 'calculated': False, 'error': None, 'top_nodes': [], 'min_score': None, 'max_score': None}
                metrics['betweenness_centrality'] = {'available': gds_available, 'calculated': False, 'error': None, 'top_nodes': [], 'min_score': None, 'max_score': None}
                metrics['closeness_centrality'] = {'available': gds_available, 'calculated': False, 'error': None, 'top_nodes': [], 'min_score': None, 'max_score': None}

                # --- GDS 指标计算 ---
                if gds_available and metrics['node_count'] > 0: # 仅当 GDS 可用且图不为空时计算
                    logger.info("开始使用 GDS 计算图指标...")
                    projected_graph_name = f"kg_metrics_graph_{graph_id}_{int(time.time())}"
                    gds_graph_projected = False # Flag to track if projection was successful
                    ...
                    try:
                        ...
                        """
                        project_result = session.run(projection_query, graph_id=graph_id, graph_name=projected_graph_name).single()

                        if project_result and project_result['nodeCount'] > 0:
                            gds_graph_projected = True
                            logger.info(f"GDS 图 '{projected_graph_name}' 创建成功 ({project_result['nodeCount']} nodes, {project_result['relationshipCount']} relationships).")
                            ...
                            if metrics['relationship_count'] > 0: # 社区发现需要关系
                                ...
                                try:
                                    ...
                                    """
                                    community_results = session.run(louvain_query, graph_name=projected_graph_name)
                                    community_stats = {}
                                    total_communities = 0
                                    community_sizes = []
                                    ...
                                    if total_communities > 0:
                                        metrics['community_detection'].update({
                                            ...
                                            'avg_community_size': round(np.mean(community_sizes), 2) if community_sizes else 0,
                                            'median_community_size': int(np.median(community_sizes)) if community_sizes else 0,
                                        })
                                        logger.info(f"GDS 社区发现完成，发现 {total_communities} 个社区。")
                                    else:
                                        metrics['community_detection']['error'] = "Louvain 算法未返回社区结果。"
                                        logger.warning(f"图谱 {graph_id} 的 Louvain 算法未返回社区结果。")
                                except ClientError as ce:
                                    metrics['community_detection']['error'] = f"GDS Louvain 执行错误: {ce}"
                                    logger.error(f"图谱 {graph_id} GDS Louvain 失败: {ce}")
                                except Exception as e:
                                     metrics['community_detection']['error'] = f"社区发现时发生未知错误: {e}"
                                     logger.error(f"图谱 {graph_id} 社区发现时发生未知错误: {e}")
                            else:
                                 metrics['community_detection']['error'] = "图谱没有关系，无法进行社区发现。"
                            ...
                            try:
                                ...
                                degree_global_query = """
                                    ...
                                    LIMIT 10
                                """
                                global_top_degree_nodes = [record.data() for record in session.run(degree_global_query, graph_name=projected_graph_name)]
                                ...
                                """
                                degree_pool_nodes = [record.data() for record in session.run(degree_pool_query, graph_name=projected_graph_name)]
                                ...
                                """, graph_name=projected_graph_name).single()
                                degree_min_max = (degree_stats_res['min_score'], degree_stats_res['max_score']) if degree_stats_res else (None, None)
                                ...
                                logger.info(f"GDS 度中心性计算完成。Min: {degree_min_max[0]}, Max: {degree_min_max[1]}, Global/Per-Type Top fetched.")
                            except ClientError as ce:
                                metrics['degree_centrality']['error'] = f"GDS Degree Centrality 执行错误: {ce}"
                                logger.error(f"图谱 {graph_id} GDS Degree Centrality 失败: {ce}")
                            except Exception as e:
                                metrics['degree_centrality']['error'] = f"度中心性计算时发生未知错误: {e}"
                                logger.error(f"图谱 {graph_id} 度中心性计算时发生未知错误: {e}")
                            ...
                            try:
                                ...
                                betweenness_global_query = """
                                    ...
                                    LIMIT 10
                                """
                                global_top_betweenness_nodes = [record.data() for record in session.run(betweenness_global_query, graph_name=projected_graph_name)]
                                ...
                                betweenness_pool_query = """
                                    ...
                                    LIMIT 5000 // Pool size for processing
                                """
                                betweenness_pool_nodes = [record.data() for record in session.run(betweenness_pool_query, graph_name=projected_graph_name)]
                                ...
                                for node_type, nodes in nodes_grouped_by_type.items():
                                    ...
                                    top_betweenness_nodes_by_type[node_type] = nodes[:10]
                                ...
                                betweenness_stats_res = session.run("""
                                    ...
                                    RETURN centralityDistribution.min as min_score, centralityDistribution.max as max_score
                                """, graph_name=projected_graph_name).single()
                                betweenness_min_max = (betweenness_stats_res['min_score'], betweenness_stats_res['max_score']) if betweenness_stats_res else (None, None)
                                ...
                                logger.info(f"GDS 中间中心性计算完成。Min: {betweenness_min_max[0]}, Max: {betweenness_min_max[1]}, Global/Per-Type Top fetched.")
                            except ClientError as ce:
                                metrics['betweenness_centrality']['error'] = f"GDS Betweenness Centrality 执行错误: {ce}"
                                logger.error(f"图谱 {graph_id} GDS Betweenness Centrality 失败: {ce}")
                            except Exception as e:
                                metrics['betweenness_centrality']['error'] = f"中间中心性计算时发生未知错误: {e}"
                                logger.error(f"图谱 {graph_id} 中间中心性计算时发生未知错误: {e}")
                            ...
                            try:
                                ...
                                closeness_global_query = """
                                    ...
                                    LIMIT 10
                                """
                                global_top_closeness_nodes = [record.data() for record in session.run(closeness_global_query, graph_name=projected_graph_name)]
                                ...
                                closeness_pool_query = """
                                    ...
                                    LIMIT 5000 // Pool size for processing
                                """
                                closeness_pool_nodes = [record.data() for record in session.run(closeness_pool_query, graph_name=projected_graph_name)]
                                ...
                                closeness_stats_res = session.run("""
                                    ...
                                    RETURN centralityDistribution.min as min_score, centralityDistribution.max as max_score
                                """, graph_name=projected_graph_name).single()
                                closeness_min_max = (closeness_stats_res['min_score'], closeness_stats_res['max_score']) if closeness_stats_res else (None, None)
                                # Adjust NaN Min/Max if necessary
                                if closeness_min_max[0] is not None and np.isnan(closeness_min_max[0]): closeness_min_max = (0.0, closeness_min_max[1])
                                if closeness_min_max[1] is not None and np.isnan(closeness_min_max[1]): closeness_min_max = (closeness_min_max[0], 0.0)
                                ...
                                logger.info(f"GDS 接近中心性计算完成。Min: {closeness_min_max[0]}, Max: {closeness_min_max[1]}, Global/Per-Type Top fetched.")
                            except ClientError as ce:
                                # Check if error is related to disconnected graph if needed
                                metrics['closeness_centrality']['error'] = f"GDS Closeness Centrality 执行错误: {ce}"
                                logger.error(f"图谱 {graph_id} GDS Closeness Centrality 失败: {ce}")
                            except Exception as e:
                                metrics['closeness_centrality']['error'] = f"接近中心性计算时发生未知错误: {e}"
                                logger.error(f"图谱 {graph_id} 接近中心性计算时发生未知错误: {e}")
                        ...
                        # 清理 GDS 临时图
                        if gds_graph_projected: # Only drop if projection succeeded initially
                            try:
                                session.run("CALL gds.graph.drop($graph_name, false)", graph_name=projected_graph_name)
                                logger.info(f"GDS 临时图 '{projected_graph_name}' 已删除。")
                            except Exception as drop_e:
                                logger.warning(f"删除 GDS 临时图 '{projected_graph_name}' 失败: {drop_e}")
    ...
    def generate_llm_analysis(self, metrics):
        ...
        if language == 'zh-CN':
            ...
            - Community detection (using {community_info.get('algorithm', 'unknown')} algorithm):
                ...
                """
                 ...
                 prompt += "- Community detection: Not executed (GDS plugin not available)\n"
            elif community_info.get('error'):
        ...
        try:
            ...
            try:
                ...
                logger.warning(f"使用VannaManager.generate_text失败: {str(vanna_e)}，尝试直接调用")
            
            # 备用方案：根据配置的模型类型直接调用API
            if hasattr(self.vanna_manager, 'config') and hasattr(self.vanna_manager.config, 'model'):
                ...
                if model_type == 'openai':
                    ...
                    try:
                        ...
                        if hasattr(self.vanna_manager, 'vn') and hasattr(self.vanna_manager.vn, 'client'):
                            client = self.vanna_manager.vn.client
                            model_name = self.vanna_manager.config.model.model_name or "gpt-3.5-turbo"
                            ...
                            if client:
                                ...
                                response = safe_create_completion_for_kg(client, completion_params, model_name)
                    ...
                    try:
                        ...
                        if language == 'zh-CN':
                            ...
                            default_response = "Unable to get analysis results from Ollama model."
    ...
    def generate_kg_construction_suggestions(self, table_name, schema, database_config=None):
        ...
        try:
            ...
            try:
                ...
                if database_config:
                    ...
                    try:
                        sample_df = self.vanna_manager.vn.run_sql(f"SELECT * FROM {table_name} LIMIT 10")
                        ...
                        logger.warning(f"使用vanna_manager获取样本数据失败: {str(vanna_e)}")
            ...
            # 构建用于LLM的提示文本（包含样本数据）
            prompt = self._build_llm_construction_prompt(table_name, schema, sample_data)
            # 获取当前语言设置
            config = self.vanna_manager.get_config()
            language = config.get('language', {}).get('language', 'zh-CN')
            ...
            if language == 'zh-CN':
                ...
                logger.info(f"Knowledge graph construction, LLM response: {llm_response}")
            # 解析LLM回复内容
            return self._parse_kg_construction_suggestions(llm_response, table_name, schema)
    ...
    def _parse_kg_construction_suggestions(self, llm_response, table_name, schema):
        ...
        try:
            ...
            if 'node_types' not in suggestions or not isinstance(suggestions['node_types'], list):
                suggestions['node_types'] = []
                
            # 确保relationships字段存在且为列表
            if 'relationships' not in suggestions or not isinstance(suggestions['relationships'], list):
            ...
            for i, node_type in enumerate(suggestions['node_types']):
                ...
                if 'split_config' not in node_type or not isinstance(node_type['split_config'], dict):
            ...
            for i, relationship in enumerate(suggestions['relationships']):
                # 确保必要字段存在
                if 'source_node_type' not in relationship or not relationship['source_node_type']:
                    # 查找第一个节点类型作为默认值
                    if suggestions['node_types']:
                        relationship['source_node_type'] = suggestions['node_types'][0]['name']
                    else:
                        if language == 'zh-CN':
                ...
                if 'target_node_type' not in relationship or not relationship['target_node_type']:
                    # 查找第二个节点类型作为默认值，如果没有则使用第一个
                    if len(suggestions['node_types']) > 1:
                        relationship['target_node_type'] = suggestions['node_types'][1]['name']
                    elif suggestions['node_types']:
                        relationship['target_node_type'] = suggestions['node_types'][0]['name']
                    else:
                        if language == 'zh-CN':
                ...
                # 如果是跨行匹配但没有配置，添加默认配置
                if relationship['matching_mode'] == 'inter-row' and ('inter_row_options' not in relationship or not isinstance(relationship['inter_row_options'], dict)):
                    relationship['inter_row_options'] = {'grouping_columns': []}
                
                # 验证和标准化跨行匹配规则
                if relationship['matching_mode'] == 'inter-row' and 'inter_row_options' in relationship:
                    ...
                    if 'grouping_columns' not in inter_row_options:
                        ...
                        inter_row_options['grouping_columns'] = [inter_row_options['grouping_columns']]
                    
                    # 分组列现在是可选的，为空是正常行为，不需要智能推荐
                    # 移除原有的智能推荐逻辑
                    if not inter_row_options['grouping_columns']:
                        if language == 'zh-CN':
                            logger.info(f"关系 '{relationship['type']}' 是跨行匹配且无分组列，将在全表范围内匹配")
                        else:
                            logger.info(f"Relationship '{relationship['type']}' is cross-row matching with no grouping columns, will match across the entire table")
                    else:
                        if language == 'zh-CN':
                            logger.info(f"关系 '{relationship['type']}' 是跨行匹配，分组列: {inter_row_options['grouping_columns']}")
                        else:
                            logger.info(f"Relationship '{relationship['type']}' is cross-row matching, grouping columns: {inter_row_options['grouping_columns']}")
                    
                    # 验证和标准化rules
                    if 'rules' in inter_row_options and isinstance(inter_row_options['rules'], list):
                        # **新增：智能修正错误的规则配置**
                        corrected_rules = self._smart_correct_rules(inter_row_options['rules'], relationship['type'])
                        ...
                        for j, rule in enumerate(inter_row_options['rules']):
                            ...
                            if rule['type'] == 'rule':
                                ...
                                if 'operator' not in rule:
                                    rule['operator'] = '=='
                                if 'value' not in rule and rule['operator'] not in ['is_null', 'is_not_null']:
                                    rule['value'] = ''
                            ...
                            if j < len(inter_row_options['rules']) - 1 and 'logic_operator' not in rule:
            ...
            if language == 'zh-CN':
                ...
                logger.info(f"Knowledge graph construction, LLM response after normalization: {suggestions}")
            # 验证和规范化完成
            return suggestions
            
        except json.JSONDecodeError as e:
            logger.error(f"无法解析LLM返回的JSON: {str(e)}\n响应内容: {llm_response[:500]}...")
            # 获取当前语言设置
            config = self.vanna_manager.get_config()
            language = config.get('language', {}).get('language', 'zh-CN')
    ...
    def _build_llm_construction_prompt(self, table_name, schema, sample_data=None):
        ...
        for column in schema:
            if language == 'zh-CN':
                ...
                primary_key = "Primary Key" if column.get('primary_key', False) else "Not Primary Key"
            ...
            if columns:
                ...
                if len(sample_data) > 5:
                    if language == 'zh-CN':
                        ...
                        sample_data_text += f"... (Total {len(sample_data)} rows of sample data)\n"
        else:
            if language == 'zh-CN':
...
Please ensure the returned JSON format is completely correct, with all field names and values enclosed in double quotes."""
    ...
    def _create_entities_with_graph_label(self, table_data, entity_type, id_columns, attr_columns, 
                                         split_config=None, graph_id=None, graph_name=None):
        """
        使用图谱标签隔离的实体创建方法（备选方案）
        
        每个实体会添加额外的图谱标签，如：Graph_10, ProjectKnowledgeGraph
        """
        entities = self._create_entities(table_data, entity_type, id_columns, attr_columns, split_config)
        ...
        for entity_id, entity_data in entities.items():
            ...
            if graph_name:
                # 清理图谱名称作为标签
                clean_graph_name = re.sub(r'[^a-zA-Z0-9_]', '', graph_name.replace(' ', '_'))
    ...
    def _smart_correct_rules(self, rules, relationship_type):
        ...
        while i < len(rules):
            ...
            if (i + 1 < len(rules) and 
                current_rule.get('type') == 'rule' and 
                rules[i + 1].get('type') == 'rule' and
                current_rule.get('value') is None and 
                rules[i + 1].get('value') is None):
                ...
                if self._is_inter_entity_comparison_pattern(current_rule, next_rule):
                    ...
                    corrected_rule = self._convert_to_inter_entity_compare(current_rule, next_rule)
                    ...
                    if language == 'zh-CN':
                        logger.info(f"关系 '{relationship_type}' 的规则已智能修正：将两个无效的单实体规则转换为实体间比较规则")
                    else:
                        logger.info(f"Rules for relationship '{relationship_type}' have been intelligently corrected: converted two invalid single-entity rules to inter-entity comparison rules")
    ...
    def _is_inter_entity_comparison_pattern(self, rule1, rule2):
        ...
        """
        # 检查基本条件
        if (rule1.get('type') != 'rule' or rule2.get('type') != 'rule' or
            rule1.get('value') is not None or rule2.get('value') is not None):
            return False
