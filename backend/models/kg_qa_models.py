...
class KGQA_ChatSession(Base):
    """聊天会话模型"""
    __tablename__ = 'kg_chat_sessions'
    ...
    # 关系
    messages = relationship("KGQA_ChatMessage", back_populates="session", cascade="all, delete-orphan")
...
class KGQA_ChatMessage(Base):
    """聊天消息模型"""
    __tablename__ = 'kg_chat_messages'
    
    id = Column(Integer, primary_key=True)
    session_id = Column(Integer, ForeignKey('kg_chat_sessions.id'), nullable=False)
    role = Column(String(50), nullable=False)  # 'user' 或 'assistant'
    content = Column(Text, nullable=False)
    cypher = Column(Text)  # 生成的cypher（对于assistant消息）
    result = Column(Text)  # cypher执行结果（对于assistant消息，JSON格式存储）
    ...
    # 关系
    session = relationship("KGQA_ChatSession", back_populates="messages")
...
class KGQA_SavedQuery(Base):
    """保存的查询模型"""
    __tablename__ = 'kg_saved_queries'
