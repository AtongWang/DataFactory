...
def safe_create_completion_for_model_manager(model_name, messages, temperature=0.1, response_format=None):
    """
    ...
    try:
        ...
        if 'provider' in error_msg and ('unexpected' in error_msg or 'invalid' in error_msg):
            logger.warning(f"模型管理器: 模型 {model_name} 不支持provider参数，尝试移除provider参数后重试")
            ...
            try:
                ...
                logger.info(f"模型管理器: 模型 {model_name} 成功使用fallback请求（无provider参数）")
                return response
            except Exception as fallback_e:
                logger.error(f"模型管理器: 模型 {model_name} fallback请求也失败: {str(fallback_e)}")
                raise fallback_e
        else:
            # 其他类型的错误，直接抛出
            raise e
...
class BaseModelProvider(ABC):
    """模型提供商基类"""
    @abstractmethod
    def chat(self, messages: List[Dict[str, str]], format: Optional[Dict] = None) -> str:
        pass
...
class OpenAIProvider(BaseModelProvider):
    ...
    def _verify_model(self):
        ...
        try:
            ...
            if available_models and self.model not in available_models:
                logger.warning(f"警告: 配置的模型 '{self.model}' 不在可用模型列表中。可用模型: {', '.join(available_models[:5])}{'...' if len(available_models) > 5 else ''}")
    ...
    def chat(self, messages: List[Dict[str, str]], format: Optional[Dict] = None) -> str:
        try:
            logger = logging.getLogger(__name__)
            logger.info(f"调用OpenAI，模型: {self.model}, API Base: {openai.base_url if hasattr(openai, 'base_url') else '默认'}")
    ...
    def list_models(self) -> List[str]:
        try:
            logger = logging.getLogger(__name__)
            logger.info(f"正在获取OpenAI可用模型列表, API Base: {openai.base_url if hasattr(openai, 'base_url') else '默认'}")
...
class ModelManager:
    ...
    def analyze_columns(self, columns: List[str], sample_data: Dict[str, List[str]], total_rows: int) -> ColumnAnalysis:
        """分析列名并返回结构化结果"""
        # 获取当前语言设置
...
Please ensure the returned result is valid JSON format with "columns" as the top-level key."""
        ...
        try:
            ...
            try:
                ...
                if end_index == -1:
                    ...
                    if cleaned_response.startswith('{') and cleaned_response.endswith('}'):
                         ...
                         logger.warning("JSON object boundaries possibly unclear, using regex cleaned response")
                    else:
            ...
            if not isinstance(result, dict):
                raise ValueError(f"LLM response parsed, but it's not a dictionary. Type: {type(result)}")
            
            column_infos = [
                ColumnInfo(**col) for col in result.get("columns", [])
            ]
