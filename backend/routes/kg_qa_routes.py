...
kgqa_bp = Blueprint("kgqa", __name__)
...
# 初始化图问答链
@kgqa_bp.route("/api/kg-init-graph-qa", methods=["POST"])
def init_graph_qa():
    """初始化图数据库问答链"""
    try:
        graph_qa_chain = kgqa_manager.init_graph_qa_chain()
        return jsonify({"status": "success", "message": "图数据库问答链初始化成功"})
    except Exception as e:
        logger.error(f"初始化图数据库问答链失败: {str(e)}")
        return jsonify({"status": "error", "message": str(e)}), 500


@kgqa_bp.route("/knowledge-graph-qa")
def knowledge_graph_qa():
    return render_template("knowledge_graph_qa.html")


# 会话管理接口
@kgqa_bp.route("/api/kg-chat-sessions", methods=["GET"])
def get_chat_sessions():
    try:
        sessions = kgqa_manager.get_all_sessions()
...
@kgqa_bp.route("/api/kg-chat-sessions", methods=["POST"])
def create_chat_session():
    try:
        ...
        session = kgqa_manager.create_session(
            name=name,
...
@kgqa_bp.route("/api/kg-chat-sessions/<int:session_id>", methods=["GET"])
def get_chat_session(session_id):
    try:
        session = kgqa_manager.get_session(session_id)
        if not session:
...
@kgqa_bp.route("/api/kg-chat-sessions/<int:session_id>", methods=["PUT"])
def update_chat_session(session_id):
    try:
        data = request.json
        session = kgqa_manager.update_session(session_id, **data)
        if not session:
            return jsonify({"status": "error", "message": "会话不存在"}), 404
        return jsonify({"status": "success", "session": session})
    except Exception as e:
...
@kgqa_bp.route("/api/kg-chat-sessions/<int:session_id>", methods=["DELETE"])
def delete_chat_session(session_id):
    try:
        success = kgqa_manager.delete_session(session_id)
        if not success:
            return jsonify({"status": "error", "message": "会话不存在"}), 404
...
# 消息管理接口
@kgqa_bp.route("/api/kg-chat-sessions/<int:session_id>/messages", methods=["GET"])
def get_messages(session_id):
    try:
        messages = kgqa_manager.get_messages(session_id)
...
@kgqa_bp.route("/api/kg-chat-sessions/<int:session_id>/messages", methods=["DELETE"])
def clear_messages(session_id):
    try:
        ...
        for message in messages:
            kgqa_manager.delete_message(message["id"])
...
@kgqa_bp.route(
    "/api/kg-chat-sessions/<int:session_id>/messages/<int:message_id>",
    methods=["DELETE"],
)
def delete_message(session_id, message_id):
    try:
        success = kgqa_manager.delete_message(message_id)
        if not success:
...
# 问答接口
@kgqa_bp.route("/api/kg-ask", methods=["POST"])
def ask_question():
    """处理用户的问题"""
    try:
        data = request.json
        ...
        # 处理问题并获取回答
        response = kgqa_manager.ask_question(
...
@kgqa_bp.route("/api/kg-ask/stream", methods=["POST"])
def ask_question_stream():
    ...
    def generate():
        try:
            for event in kgqa_manager.ask_question_stream(
                session_id, question, database_name, table_name
            ):
                yield json.dumps(event, ensure_ascii=False) + "\n"
        except Exception as e:
            logger.error(f"KG 流式处理问题失败: {str(e)}")
            yield (
...
@kgqa_bp.route("/api/kg-train", methods=["POST"])
def train():
    try:
...
@kgqa_bp.route("/api/kg-ollama-models", methods=["GET"])
...
@kgqa_bp.route("/api/kg-config", methods=["GET"])
def get_config():
    """获取系统配置信息"""
    try:
...
# 保存查询相关接口
@kgqa_bp.route("/api/kg-saved-queries", methods=["GET"])
def get_saved_queries():
    try:
        queries = kgqa_manager.get_all_saved_queries()
...
@kgqa_bp.route("/api/kg-saved-queries", methods=["POST"])
def save_query():
    try:
        data = request.json
        ...
        saved_query = kgqa_manager.save_query_result(
            title=title,
            question=question,
            sql=sql,
            result=result,
...
@kgqa_bp.route("/api/kg-saved-queries/<int:query_id>", methods=["GET"])
def get_saved_query(query_id):
    try:
        query = kgqa_manager.get_query(query_id)
        if not query:
...
@kgqa_bp.route("/api/kg-saved-queries/<int:query_id>", methods=["DELETE"])
def delete_saved_query(query_id):
    try:
        success = kgqa_manager.delete_query(query_id)
        if not success:
            return jsonify({"status": "error", "message": "查询不存在"}), 404
...
@kgqa_bp.route("/api/kg-toggle-table-lock/<int:session_id>", methods=["POST"])
def toggle_table_lock(session_id):
    ...
    try:
        ...
        # 调用管理器方法切换锁定状态
        session = kgqa_manager.toggle_table_lock(session_id, lock_status)
...
@kgqa_bp.route("/api/kg-generate-session-name/<int:session_id>", methods=["POST"])
def generate_session_name(session_id):
    ...
    try:
        ...
        # 调用生成名称方法
        new_name = kgqa_manager.generate_session_name(session_id, first_message)
...
@kgqa_bp.route("/api/kg-training-data", methods=["GET"])
def get_training_data_api():
    """获取所有训练数据"""
    try:
        df = kgqa_manager.get_training_data()
        # 如果是空DataFrame
        if df.empty:
            logger.info("训练数据为空")
...
@kgqa_bp.route("/api/kg-training-data/<item_id>", methods=["DELETE"])
def delete_training_data_api(item_id):
    """删除指定的训练数据"""
    try:
        logger.info(f"尝试删除训练数据: {item_id}")
        success = kgqa_manager.delete_training_data(id=item_id)
        if success:
...
@kgqa_bp.route("/api/kg-add-sql-qa", methods=["POST"])
def add_sql_qa_api():
    """添加SQL问答对训练数据"""
    try:
        data = request.json
        ...
        if not question or not sql:
            return jsonify(
                {"status": "error", "message": "问题和Cypher语句不能为空"}
            ), 400

        # 使用vanna_manager添加SQL问答对
        kgqa_manager.add_cypher_question(question=question, query=sql)

        return jsonify({"status": "success", "message": "Cypher问答对添加成功"})
    except Exception as e:
...
@kgqa_bp.route("/api/kg-add-documentation", methods=["POST"])
def add_documentation_api():
    ...
    try:
        ...
        # 使用vanna_manager添加文档
        kgqa_manager.add_documentation(documentation)
...
@kgqa_bp.route("/api/kg-naming-settings", methods=["GET"])
...
@kgqa_bp.route("/api/kg-cypher-examples", methods=["POST"])
def add_cypher_example():
    """添加新的Cypher问答示例"""
    try:
        data = request.json
        question = data.get("question")
        ...
        if not question or not query:
            return jsonify(
                {"status": "error", "message": "问题和Cypher查询不能为空"}
            ), 400

        success = kgqa_manager.add_cypher_question(
            question=question, query=query, result=result, answer=answer
        )

        if success:
            return jsonify({"status": "success", "message": "Cypher问答示例添加成功"})
        else:
            return jsonify(
                {"status": "error", "message": "添加Cypher问答示例失败"}
            ), 500
    except Exception as e:
        logger.error(f"添加Cypher问答示例失败: {str(e)}")
        return jsonify({"status": "error", "message": str(e)}), 500
